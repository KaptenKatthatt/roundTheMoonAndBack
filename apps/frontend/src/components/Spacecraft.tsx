...
// ⚡ Bolt: Pre-allocate vectors outside render loop to prevent GC spikes
...
export function Spacecraft() {
  ...
  useFrame(() => {
    ...
    if (_posAhead.distanceTo(_scPos) > 0.001) {
      _forward.subVectors(_posAhead, _scPos).normalize();
      groupRef.current.up.crossVectors(_forward, SUN_DIR).normalize();
      groupRef.current.lookAt(_posAhead);
    }
