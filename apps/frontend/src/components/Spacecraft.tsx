...
// ⚡ Bolt: Pre-allocate vectors to avoid instantiation on every frame
...
export function Spacecraft() {
  ...
  useFrame(() => {
    ...
    getPositionAt(currentTime, _pos);
    ...
    if (currentTime >= FLYBY_START_T && currentTime <= FLYBY_END_T) {
      ...
      if (hour !== lastLoggedHour) {
        ...
        const dist = _pos.distanceTo(_moonPos);
    ...
    getPositionAt(currentTime + 60_000, _ahead);
    if (_ahead.distanceTo(_pos) > 0.001) {
      _forward.subVectors(_ahead, _pos).normalize();
      groupRef.current.up.crossVectors(_forward, SUN_DIR).normalize();
      groupRef.current.lookAt(_ahead);
    }
